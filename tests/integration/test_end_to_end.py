...
class TestRealDatasetProcessing:
    ...
    def test_process_key_samples_enhanced(self, sample_file, samples_dir, output_dir):
        ...
        print(f"  Enhanced: {stats['input_lines']} -> {stats['output_lines']} lines ({stats['reduction_percent']:.1f}% reduction)")
...
class TestProcessingModeIntegration:
    ...
    @pytest.mark.parametrize("sample_file", ["apache_access.log", "linux_system.log", "spark_application.log"])
    ...
    def test_anomaly_mode_on_system_logs(self, samples_dir):
        ...
        system_files = ["linux_system.log", "bgl_supercomputer.log", "thunderbird_hpc.log"]
    ...
    def test_temporal_mode_on_timestamped_logs(self, samples_dir):
        ...
        timestamped_files = ["apache_access.log", "openstack_nova.log", "spark_application.log"]
...
class TestComprehensiveRealDataValidation:
    ...
    def test_all_samples_process_successfully(self, samples_dir, output_dir):
        ...
        for result in processing_results:
            print(f"{result['file']:<25} {result['input_lines']:<8} {result['output_lines']:<6} "
    ...
    def test_enhanced_processing_sample_subset(self, samples_dir):
        ...
        key_samples = ["apache_access.log", "hdfs_system.log", "linux_system.log", "spark_application.log"]
        ...
        for result in enhanced_results:
            print(f"  {result['file']}: {result['reduction_percent']:.1f}% reduction "
